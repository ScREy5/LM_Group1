...
class SimulationRobobo(Robobo):
    ...
    def connect(self, address='127.0.0.1', port=19999, rendering = True):
        ...
        while time.time() - startTime < get_handles_timeout:
            try:
                ...
                print(f"setting rendering = {rendering}")
                vrep.simxSetBooleanParameter(self._clientID, vrep.sim_boolparam_display_enabled, rendering, vrep.simx_opmode_oneshot)
                # vrep.simxSetBooleanParameter(self._clientID, 25, realtime, vrep.simx_opmode_oneshot)
                return self
