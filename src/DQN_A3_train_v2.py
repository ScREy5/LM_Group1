...
class Environment():
    ...
    def get_reward(self,state):
        ...
        # Collision check test 1: Removed r/l and removed terminated status
        if np.any([back>=0.31]) or np.any([f_rl>= 0.21]) or np.any([f_rrll>=0.25]) and not self.got_closer: #collission check
            reward = -100
            self.hit_counter += 1
...
def main():
    ...
    # validate(env,f"logs/task3/agent3_v3/", None)
