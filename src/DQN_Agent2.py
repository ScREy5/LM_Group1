...
class DQN(nn.Module):
    ...
    def __init__(self, env, learning_rate):
        ...
        input_features = env.observation_space_size+3
        action_space = 3
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        if auto_save != None :
            if epoch % auto_save == 0:
                agent.save(f"logs/task2/agent2/backup/autosave_{epoch}/")
                np.savetxt(f"logs/task2/agent2/backup/autosave_{epoch}/all_steps.txt", np.array([all_steps]))
