...
class DQN(nn.Module):
    ...
    def __init__(self, env, learning_rate):
        ...
        input_features = env.observation_space_size
        action_space = 3
...
class DDQNAgent:
    ...
    def __init__(self, env, device, epsilon_decay,
        ...
        if not cold_boot:
            self.replay_memory = ExperienceReplay(self.env, self.buffer_size, min_replay_size, seed=seed)
    ...
    def load(self, Path):
        self.online_network.load_state_dict(torch.load(Path + "Q_network.pt"))
        self.target_network.load_state_dict(torch.load(Path + "T_network.pt"))
        print("Loaded successfully")


def training_loop(env, agent, max_epochs, target_=False, batch_size = 1, auto_save = False, cont = None):
    '''
    Params:
    env = name of the environment that the agent needs to play
    ...
    for epoch in range(max_epochs):
        ...
        if auto_save :
            if epoch % 1 == 0:
                agent.save(f"backup/autosave_{epoch}/")
                np.savetxt(f"backup/autosave_{epoch}/all_steps.txt", np.array([all_steps]))
